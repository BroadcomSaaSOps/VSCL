...
Log_Print "Determining the available installer version..."
unset INI_SECTION
Log_Print "Finding section for current installer version in '$LOCAL_VERSION_FILE'..."
...
Log_Print "New Installer Version Available: $PACKAGE_VERSION"

# Download the dat files...
Log_Print "Downloading the current installer '$FILE_NAME' from '$DOWNLOAD_SITE'..."
...
Log_Print "Extracting installer './$TEMP_DIR/$FILE_NAME' to directory './$TEMP_DIR'..."

if ! cd "$TEMP_DIR"; then
    Exit_WithError "Unable to change to temp directory './$TEMP_DIR'!"
fi

if ! tar -xvzf ./"$FILE_NAME"; then
